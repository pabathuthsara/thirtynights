...
import { validateReportContract, type ReportResult, type TranscriptSegment } from '../src/contracts.js';
...
describe('report worker contract', () => {
  it('accepts bounded evidence and normalized exact quotes', () => {
    ...
    expect(() => validateReportContract(candidate, { checkpoint_night: 7 }, segments, 'v1')).not.toThrow();
  });

  it('rejects a false quote', () => {
    const candidate = report();
    candidate.sections[0]!.evidence[0]!.quote = 'Everything was perfect.';
    expect(() => validateReportContract(candidate, { checkpoint_night: 7 }, segments, 'v1')).toThrow('report_false_quote');
  });

  it('rejects a citation assigned to the wrong night', () => {
    const candidate = report();
    candidate.sections[0]!.evidence[0]!.night_id = 'night-2';
    expect(() => validateReportContract(candidate, { checkpoint_night: 7 }, segments, 'v1')).toThrow('report_invalid_citation');
  });
  ...
  it('rejects missing or blank evidence quotes', () => {
    ...
    expect(() => validateReportContract(candidate, { checkpoint_night: 7 }, segments, 'v1')).toThrow('report_missing_quote');
  });

  it('rejects mismatched schema and checkpoint versions', () => {
    expect(() => validateReportContract(report(), { checkpoint_night: 30 }, segments, 'v1')).toThrow('report_contract_mismatch');
    expect(() => validateReportContract(report(), { checkpoint_night: 7 }, segments, 'v2')).toThrow('report_contract_mismatch');
  });
  ...
  it('rejects duplicate transcript segment identifiers', () => {
    expect(() => validateReportContract(report(), { checkpoint_night: 7 }, [...segments, segments[0]!], 'v1'))
